...
export function SocketProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (payload: SocketNewMessagePayload) => {
      const incoming = normalizeMessage(payload, useAuthStore.getState().user)
      if (!incoming?.conversationId) return

      useUserDirectory.getState().remember([incoming.sender])

      const viewing = typeof window !== 'undefined' && window.location.pathname === `/chat/${incoming.conversationId}`
      if (!viewing) {
      ...
      queryClient.setQueryData(
        ...
        (old: { pages: MessageHistoryResponse[] } | undefined) => {
          ...
          const alreadyPresent = old.pages.some((page) => page.messages.some((m) => m._id === incoming._id))
          if (alreadyPresent) return old

          const isEcho = (m: Message) =>
            isOptimistic(m) && m.text === incoming.text && getSenderId(m) === getSenderId(incoming)
