...
function asConversation(raw: unknown, fallback: string): Conversation {
  const conversation = normalizeConversation(unwrapObject(raw, 'conversation') ?? raw)
  if (!conversation?._id) {
...
export const groupsApi = {
  ...
  async addParticipants(conversationId: string, data: AddParticipantsRequest): Promise<Conversation> {
    const res = await apiClient.post<unknown>(`/conversations/${conversationId}/participants`, data)
  ...
  async promoteAdmin(conversationId: string, data: PromoteAdminRequest): Promise<Conversation> {
    const res = await apiClient.post<unknown>(`/conversations/${conversationId}/admins`, data)
    return asConversation(res.data, 'Could not promote that member.')
  },

  async rename(conversationId: string, data: RenameGroupRequest): Promise<Conversation> {
    const res = await apiClient.patch<unknown>(`/conversations/${conversationId}`, data)
