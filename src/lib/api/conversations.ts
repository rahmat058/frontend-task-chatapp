...
import { hydrateConversation, normalizeConversation } from '@/lib/utils/conversation'
import type {
...
export const conversationsApi = {
  async list(): Promise<ListConversationsResponse> {
    ...
    useUserDirectory
      .getState()
      .remember(
        conversations.flatMap((conversation) => [...conversation.participants, conversation.lastMessage?.sender]),
      )

    return conversations.map(attachKnownNames)
  },

  async startDirect(data: StartConversationRequest): Promise<StartConversationResponse> {
    const res = await apiClient.post<unknown>('/conversations', data)
    const conversation = normalizeConversation(unwrapObject(res.data, 'conversation'))
  ...
  async getMessages(
    ...
    const res = await apiClient.get<unknown>(`/conversations/${conversationId}/messages`, { params })
