...
function participantFromUnknown(value: unknown): Participant | null {
  ...
  const name =
    typeof nested.name === 'string' ? nested.name.trim() : typeof obj.name === 'string' ? obj.name.trim() : ''
  const phone = typeof nested.phone === 'string' ? nested.phone : typeof obj.phone === 'string' ? obj.phone : ''
...
function collectRawParticipants(raw: Record<string, unknown>): unknown[] {
  const buckets = [raw.participants, raw.members, raw.users, raw.userIds, raw.participantIds]
...
export function normalizeConversation(raw: unknown): Conversation | null {
  ...
  const type: ConversationType = nested.type === 'group' || nested.isGroup === true ? 'group' : 'direct'
  ...
  const lastSender = lastMessageRaw ? participantFromUnknown(lastMessageRaw.sender) : null
  const lastMessage = lastMessageRaw
    ? {
        ...
        createdAt: typeof lastMessageRaw.createdAt === 'string' ? lastMessageRaw.createdAt : undefined,
      }
    : undefined
...
export function getParticipantIds(conversation: Conversation): string[] {
  const raw = conversation.participants
  if (!Array.isArray(raw)) return []
  return raw.map((p) => (typeof p === 'string' ? p.trim() : getEntityId(p))).filter((id): id is string => Boolean(id))
}
...
export function getOtherParticipantId(conversation: Conversation, currentUserId?: string): string | undefined {
...
export function getConversationName(
  ...
  const populated = getParticipants(conversation).find((p) => p._id !== currentUserId)
  ...
  const senderName = sender && typeof sender === 'object' && typeof sender.name === 'string' ? sender.name.trim() : ''
...
export function sortByRecency(conversations: Conversation[]): Conversation[] {
  return [...conversations].sort((a, b) => toTimestamp(getLastActivity(b)) - toTimestamp(getLastActivity(a)))
}
