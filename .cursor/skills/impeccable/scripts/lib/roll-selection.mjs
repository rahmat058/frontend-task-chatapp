...
function challengerTickets(pool) {
  return pool.flatMap((concept) => {
    ...
    return Array.from({ length: ticketsForRating(concept.review?.rating) }, (_, ticket) => ({ concept, ticket }))
  })
}

function compositionTickets(pool) {
  return pool.flatMap((composition) =>
    Array.from({ length: ticketsForRating(composition.review?.rating) }, (_, ticket) => ({ composition, ticket })),
  )
}
...
export function* selectApprovedChallengers({ scope, key, reroll = 0, minRating = null, mode = null, concepts }) {
  ...
  function* pickRound(round, excluded) {
    ...
    for (const [index, tier] of tierOrder.entries()) {
      ...
      const first = order[0]
      const second =
        order.find((concept) => concept.familyId !== first.familyId) || order.find((concept) => concept.id !== first.id)
