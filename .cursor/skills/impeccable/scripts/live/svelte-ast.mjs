  ...
  switch (node.type) {
    ...
    default: {
      for (const key of Object.keys(node)) {
        if (key === 'type' || key === 'start' || key === 'end' || key === 'loc' || key === 'range' || key === 'parent')
          continue
...
export function derivePropName(expr) {
  ...
  const candidate = (tail && tail[1]) || (String(expr).match(/^([A-Za-z_$][\w$]*)$/) || [])[1] || 'value'
...
export function buildPropsScriptV2(contract) {
  ...
  const typeFields = contract.map((c) => `    ${c.prop}?: ${types[c.kind] ?? 'string'};`).join('\n')
  return `<script>\n  /** @typedef {{\n${typeFields}\n  }} Props */\n  let { ${names} } = $props();\n</script>\n`
}
