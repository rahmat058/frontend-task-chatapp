...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
  ...
  const absTarget = targetPath ? (path.isAbsolute(targetPath) ? targetPath : path.resolve(absCwd, targetPath)) : null
  ...
  const repoRoot = targetGitRoot || (cwdGitRoot && insideOrEqual(targetDir, cwdGitRoot) ? cwdGitRoot : null)
  ...
  const markerBound =
    absTarget && insideOrEqual(targetDir, legacyRoot) && insideOrEqual(legacyRoot, upperBound) ? legacyRoot : upperBound

  let appRoot = walkUp(targetDir, markerBound, (dir) => (isAppRoot(dir) ? dir : null))
  let resolvedFrom = appRoot ? (absTarget ? `target:${path.relative(absCwd, absTarget) || '.'}` : 'cwd') : null
  ...
  const contextRoot = productPath ? path.dirname(productPath) : designPath ? path.dirname(designPath) : null
    ...
    if (gitRoot) {
      ...
      if (candidates.length > 0) {
        ...
        const recoveringApps =
          liveApps.length > 0 ? liveApps : candidates.filter((manifest) => hasActiveDurableSession(manifest.appRoot))
