...
export function parseReplyArgs(args) {
  ...
  const message =
    args.find((a, i) => i > replyIdx + 2 && !a.startsWith('--') && i !== fileIdx + 1 && i !== dataIdx + 1) || undefined
...
export async function postReply(base, token, reply) {
  ...
  if (!res.ok) {
    ...
    const failureLines = Array.isArray(body.failures)
      ...
      : null
    const parts = [body.error || res.statusText, body.reason, body.hint, failureLines, body._instructions].filter(
      Boolean,
    )
