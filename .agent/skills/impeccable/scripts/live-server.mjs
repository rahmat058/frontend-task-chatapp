...
function resolveProjectContext() {
  ...
  const designPath = ctx.designPath
    ...
    : LIVE_ROOTS?.designPath && fs.existsSync(LIVE_ROOTS.designPath)
      ...
      : null
  const hasProduct = ctx.hasProduct || !!(LIVE_ROOTS?.productPath && fs.existsSync(LIVE_ROOTS.productPath))
  return {
    ...ctx,
    hasProduct,
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      const jsonPath =
        resolveDesignSidecarPath(process.cwd(), projectContext.designContextDir || projectContext.contextDir) ||
    ...
    if (p === '/events' && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        if (msg.type === 'manual_edit_apply') {
          res.writeHead(400, { 'Content-Type': 'application/json' })
          res.end(
            JSON.stringify({ error: 'manual_edit_apply is disabled; use /manual-edit-stash then /manual-edit-commit' }),
          )
          return
        }
...
function handlePollGet(req, res, url) {
  ...
  if (available) {
    ...
    leaseEvent(available, leaseMs).then(
      ...
      (error) => {
        console.error(
          '[live] lease failed for ' + (available.event?.id || 'unknown') + ': ' + (error?.message || error),
        )
        if (res.writableEnded || res.destroyed) return
        res.writeHead(200, { 'Content-Type': 'application/json' })
...
function handlePollPost(req, res) {
  ...
  req.on('end', () => {
    ...
    if (replyFileMeta.previewMode === 'svelte-component' && msg.id && (msg.type === 'done' || !msg.type)) {
...
function sweepOrphanSvelteComponentSessionsOnStartup() {
  try {
    const activeIds = (state.sessionStore?.listActiveSessions() || []).map((snapshot) => snapshot?.id).filter(Boolean)
